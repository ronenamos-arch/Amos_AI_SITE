...
### Stage 3 — Alt Text ✅
- Audited all `<img>` and `<Image>` tags across `app/` and `components/` — all public-facing images already had descriptive Hebrew alt text. No changes needed.
...
### Still TODO (SEO)
- [ ] Backlinks: guest posts on finance/accounting blogs, Israeli CPA directories (manual, ongoing)
- [ ] Site speed: PageSpeed Insights audit, compress blog post cover images
- [ ] Set `meta_title` on existing DB blog posts via admin edit page (optional, do per post)
