...
- **Categories:** Codex, ChatGPT, אוטומציה, Excel, Power BI, NotebookLM, מחלקות כספים
...
Admin page at `/admin/preview-emails` — split-pane UI: live iframe preview (left) + HTML editor (right).

**How it works:**
- 4 drip emails: `welcome`, `day3`, `day7`, `day14`
- Default HTML is generated by `build*Email()` functions in `lib/emails/*.ts`
- Admin can edit the HTML and save — stored in the `email_templates` Supabase table (key PK, html, updated_at)
- Preview and editor always show DB override if it exists, otherwise fall back to the code builder
- "מותאם אישית" badge shown when a DB override is active; "אפס" button deletes it and restores the default
- `X-Frame-Options: SAMEORIGIN` override in `next.config.ts` for `/api/admin/preview-email` (global header is DENY)

**API routes (all admin-gated, no `"use server"`):**
- `GET /api/admin/preview-email?email=<key>` — returns rendered HTML (DB → fallback to builder)
- `GET /api/admin/get-email-source?email=<key>` — returns `{ html, isCustom }` for the editor
- `POST /api/admin/save-email` body `{ email, content }` — upserts HTML to `email_templates`
- `DELETE /api/admin/save-email?email=<key>` — removes DB override, restores default

**Lesson learned:** `fs.writeFileSync` doesn't work on Vercel — filesystem is read-only in serverless. Always use Supabase for persistent admin edits.
...
- [x] Admin email preview & edit page (`/admin/preview-emails`) — split-pane iframe preview + HTML editor, saves to `email_templates` Supabase table, reset-to-default button
...
## Bing SEO Audit — Completed (2026-04-01)

Full audit from Bing Webmaster Tools. 22 warnings found. All 4 stages fixed.

### Stage 1 — Title Tags ✅
- `app/about/page.tsx`, `app/services/page.tsx`, `app/contact/page.tsx`, `app/blog/page.tsx`, `app/faq/layout.tsx` — all use `title: { absolute: '...' }` to prevent layout template double-appending `| רונן עמוס`
export const dynamic = 'force-dynamic'
```
...
### Stage 1 — Title Tags ✅
- `app/about/page.tsx`, `app/services/page.tsx`, `app/contact/page.tsx`, `app/blog/page.tsx`, `app/faq/layout.tsx` — all use `title: { absolute: '...' }` to prevent layout template double-appending `| רונן עמוס`
- `articles` DB table — `meta_title TEXT` column added (migration: `20260401000000_add_meta_title_to_articles.sql`)
- `lib/blog-supabase.ts` — `DBArticle` interface includes `meta_title: string | null`
- `app/blog/[slug]/page.tsx` `generateMetadata` — uses `meta_title` when set, falls back to `title`
- Admin edit form (`app/admin/blog/edit/[slug]/EditArticleClient.tsx`) — "כותרת SEO" field with 60-char counter

### Stage 2 — H1 Structure ✅
- `components/ui/SectionHeading.tsx` — added `as?: 'h1' | 'h2'` prop (defaults to `h2`, no breaking change)
- Fixed 5 pages missing H1: `app/services`, `app/contact`, `app/training`, `app/blog`, `app/pricing` — pass `as="h1"` to first SectionHeading
- Already had H1: `app/about`, `app/courses/ai-mastery`, `app/courses/notebook-master`, `app/skill-vault`

### Stage 3 — Alt Text ✅
- Audited all `<img>` and `<Image>` tags across `app/` and `components/` — all public-facing images already had descriptive Hebrew alt text.

### Stage 4 — IndexNow ✅
- `lib/indexnow.ts` — `pingIndexNow(slug)` fires GET to `https://api.indexnow.org/indexnow` (key hardcoded: `f9826b1b81c34964b0fa14797b4af314`)
- `public/f9826b1b81c34964b0fa14797b4af314.txt` — key verification file
- `lib/actions/articles.ts` — fires after both `createArticle` and `updateArticle`

## SEO & GEO (Generative Engine Optimization) Auditing

- **Automated Zero-Token Audit Engine:** `scripts/seo_geo_audit.py`
  - Runs 12 deterministic technical SEO & GEO citability checks in <0.1s at 0 token cost.
  - Checks: Title/slug length, excerpt length (120-160 chars), H1 single hierarchy, keyword placement (intro/H2), image alts, internal links, zero-click answer blocks, list density, numeric stats density, E-E-A-T credentials, and `public/llms.txt` sync.
  - Usage: `& 'C:\Python313\python.exe' scripts\seo_geo_audit.py "content\posts\filename.md" --keyword "רואה חשבון AI"`
  - Bulk audit: `& 'C:\Python313\python.exe' scripts\seo_geo_audit.py --all`
  - Auto-sync to llms.txt: add `--sync-llms`
- **Google Search Console Client:** `scripts/gsc_client.py`
  - Verified Service Account with `siteOwner` permission on `sc-domain:ronenamoscpa.co.il`.
  - Commands: `sites`, `sitemaps`, `inspect <url>`, `queries --days 28`, `pages --days 28`, `submit-sitemap <url>`.
- **Pre-Publish Skill:** `seo-geo-publish` (`C:\Users\Ronen\.gemini\config\skills\seo-geo-publish\SKILL.md`)
## Language
...
*Auto-generated by `headroom learn` on 2026-06-30 — do not edit manually*

### Image Analysis
*~1,361,280 tokens/session saved*
- Never re-call `view_image` on a path already viewed in the current session — use your in-context analysis. The same image was re-loaded 19× in one session, wasting ~1.36M tokens.

### File Re-reads
*~49,055 tokens/session saved*
- Do not re-read a file with `Get-Content -Raw` unless it was modified since last read. The APPscripts files were re-fetched 30× in one session (~49K tokens wasted). Check whether the content is already in context before issuing a new read.

### Commands — Timeouts
*~4,039 tokens/session saved*
- `Get-ChildItem -Recurse` on broad roots (`C:\Users\Ronen`, `C:\Program Files`, `.\` when node_modules is present) consistently times out. Always scope to a specific project subdirectory, or use `rg --files` with glob filters instead.

### Directory Listings
*~4,039 tokens/session saved*
- `Get-ChildItem -Recurse -File` on project roots was re-issued 7× per session. Run it once at session start and reference the cached listing; re-run only after structural file changes (add/delete).

### Environment — Python
*~2,214 tokens/session saved*
- Python is at `C:\Python313\python.exe`. Use `& 'C:\Python313\python.exe'` directly. Before running any skill script, verify required modules with `& 'C:\Python313\python.exe' -c "import pptx, PIL"` — skipping this caused a 14-iteration error loop.

### Environment — Node / clasp
*~929 tokens/session saved*
- `clasp` and `npx` require the nodejs directory prepended to PATH every shell call on this machine. Always use: `$env:PATH='C:\Program Files\nodejs;' + $env:PATH; & 'C:\Program Files\nodejs\npx.cmd' clasp <cmd>`. This pattern was needed 23× across sessions.

### Commands — rg regex
*~800 tokens/session saved*
- `rg` on Windows does not support PCRE lookahead/lookbehind by default; patterns with `(?!...)` fail with a parse error. Add `-P` flag for PCRE support, or rewrite with simpler patterns piped to `Where-Object`.
